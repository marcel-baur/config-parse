...
pub fn parse_yaml(path: &str) {
    ...
    if let Some(stringified) = handle_yaml(doc["value"].clone()) {
        println!("Some value: {}", stringified);
    }
    println!("{:?}", doc["security"]);
}
...
fn handle_yaml_hash(yaml: Hash) -> Option<String> {
    if yaml.len() > 1 {
        return Some("The yaml path you provided is ambiguous! Please specify
            the exact key!".to_string());
    }
